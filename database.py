...
def _add_missing_columns(conn, table, column_defs):
    """
    Additive column migration: CREATE TABLE IF NOT EXISTS only helps when
    a table doesn't exist at all — it does nothing if the table already
    exists but is missing a column that a newer version of the code
    added (exactly what happened here: an old 'messages' table existed
    on the live database from before 'target_desc' was introduced, so
    IF NOT EXISTS silently left it as-is). This checks the table's real
    columns via PRAGMA table_info and ALTERs in anything missing.
    column_defs: list of (column_name, sql_type_and_default) tuples.
    """
...
def ensure_messaging_tables(conn):
    ...
    conn.commit()
    # Covers the case where these tables already existed on disk from an
    # older version of the schema, missing columns added since.
    _add_missing_columns(conn, "messages", [
        ("target_desc", "target_desc TEXT NOT NULL DEFAULT ''"),
...
def ensure_reset_requests_table(conn):
    ...
    conn.commit()
    # Same defensive migration as messaging — this table's shape changed
    # more than once earlier in development, so cover any older version
    # of it that might already exist on a live database.
    _add_missing_columns(conn, "password_reset_requests", [
        ("requester_type", "requester_type TEXT NOT NULL DEFAULT 'student'"),
